---
title: "Introduction to fhiplot"
author: "Richard White, Chi Zhang"
date: "`r Sys.Date()`"
output:
  rmarkdown::html_vignette:
    ...
    fig_height: 6
vignette: >
  %\VignetteIndexEntry{Introduction to fhiplot}
...
- Use `q <- q + fhiplot::theme_fhi_lines_horizontal()`
- Use `q <- q + geom_col(fill = fhiplot::base_color, width = 0.8)`
- For `scale_y_continuous` use `breaks = fhiplot::pretty_breaks(5)`
- For `scale_y_continuous` use `expand = expansion(mult = c(0, 0.1))`
- If the x-axis is too cramped, make the labels vertical `q <- q + fhiplot::set_x_axis_vertical()`
