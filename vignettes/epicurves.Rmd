...
d[, isoyear_isoweek := fhiplot::isoyear_isoweek(date)]
...
```{r}
p <- ggplot(w[location_code=="county01"], aes(x = isoyear_isoweek, y = N))
p <- p + geom_col(fill = fhiplot::base_color, width = 0.8)
...
```{r}
p <- ggplot(w, aes(x = isoyear_isoweek, y = N))
p <- p + geom_col(fill = fhiplot::base_color, width = 0.8)
p <- p + scale_x_discrete("Isoweek")
p <- p + scale_y_continuous("Number of reported cases",
  breaks = fhiplot::pretty_breaks(5),
...
```{r fig.height=10, fig.width=8}
p <- ggplot(d, aes(x = isoyear_isoweek, y = N))
p <- p + geom_col(fill = fhiplot::base_color, width = 0.8)
